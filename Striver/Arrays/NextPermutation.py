...
def nextPerm(arr):
  ...
  for i in range(len(arr)):
    if (arr[i] >= rightMax[i]): continue
    else: maxSwappableIndex = i
  ...
  arr[maxSwappableIndex], arr[minSwapIndex] = arr[minSwapIndex], arr[maxSwappableIndex]

  # sort the remaining array (dry run to understand)
  remSorted = arr[0: maxSwappableIndex + 1] + sorted(arr[maxSwappableIndex + 1: len(arr)])
